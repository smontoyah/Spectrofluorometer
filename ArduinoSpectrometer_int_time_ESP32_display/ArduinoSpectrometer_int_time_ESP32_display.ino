...
//Future upgrade: change lmits by bluetooth
//Upgrades:
...
//Fitting coeficients (no implemented yet
int coefficient=3;
int slope=819;
int divider=584;
...
void loop(){
  ...
  if(display_time>1000){ //Update display each second
      ...
      float result=(coefficient*average+slope)/divider;//Result in the GPT scale, not working yet
      String string_result;
      if(average<MIN_average){//Limit to show the "cuvette holder empty" message
        string_result="VACIO!";
        tft.fillRect(25, 80, 200, 10, ST77XX_BLACK); //Clear previous indicator
      }else if(average<fluo_minimum){
        ...
        string_result="< 4.5 GPT";
      }else if(average<fluo_maximum){
         //Print the bar indicator
        int barWidth = map(average, MIN_average, MAX_average, 0, 200);
        tft.fillRect(25, 80, 200, 10, ST77XX_BLACK); //Clear previous indicator
